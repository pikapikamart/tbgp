...
type VerificationsListWrapperProps = {
  customed?: boolean
}

export const VerificationsListWrapper = styled.ul<VerificationsListWrapperProps>`
  margin-top: ${ rem(24) };
  outline: none;
  ...
  ${ ({ customed }) => css`
    ${ VerificationItem } {
      ...
      ${ breakpoint("desktop", `
        grid-template-columns: repeat(2, auto) ${ customed? rem(144) : "repeat(2, min-content)" };
        grid-template-rows: auto;
        padding: ${ rem(12) } ${ rem(24) };

        ${ VerificationOption }:last-of-type {
          &:first-of-type {
            grid-column: 3 / 4;
            grid-row: 1 / 3;
            margin-bottom: 0;
